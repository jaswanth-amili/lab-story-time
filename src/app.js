...
let moreAboutHome = (address, distanceFromTown, hasNeighbours) => {
	let addressType = typeof address,
		distanceType = typeof distanceFromTown,
		neighboursType = typeof hasNeighbours;
...
let moreAboutKaren = (parents, noOfSiblings, isNuclearFamily) => {
	...
	let parentType = typeof parents === "string",
		siblingsType = typeof noOfSiblings === "number",
		familyType = typeof isNuclearFamily === "boolean";
...
let doesFriendExist = (ageInText, ageInNumber) => {
	...
	return isNaN(ageInText) === true ? ageInText : ageInNumber;
};
//Progression 4:
...
let sweetTooth = (
	totalNoOfSweets = "default",
	sweetsConsumedByKaren,
	sweetsConsumedInNMeters,
	metersToTravel
) => {
	let sweetsShared = 0;
	let ele1 = typeof totalNoOfSweets,
		ele2 = typeof sweetsConsumedByKaren,
		ele3 = typeof sweetsConsumedInNMeters,
		ele4 = typeof metersToTravel;
	if (
		ele1 != "number" ||
		ele2 != "number" ||
		ele3 != "number" ||
...
let convertToCelsius = (fahrenheit) => {
	let output = 0;
	if (typeof fahrenheit !== "number") output = "Technical Error!";
...
let aDifficultChoice = (choice) => {
	return choice === 1
		...
		: choice === -1
		? "Break down and give up all hope"
		: choice === "I give up"
		? "Refused to do anything for Karen"
